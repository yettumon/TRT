...
print(f"Python {platform.python_version()} | numpy {np.__version__} | scipy {_scipy.__version__}")
...
for path in _PATHS:
    if os.path.exists(path):
        ...
        break
if df is None:
    raise FileNotFoundError("Pantheon+SH0ES.dat 없음. 다운로드: https://pantheonplussh0es.github.io/")

c_light = 299792.458

# ── GCOD axis (fixed a priori, TRT Section 2.3) ───────
...
# sin(θ) 선택 근거:
# TRT predicts maximum radial force effect near transverse directions (θ=90°),
# therefore sin(θ) is used as the directional basis function.
# cos(θ) would be maximal at θ=0° (axis), which is opposite to TRT prediction.

# 다중공선성 확인 (심사자 대응)
rho_z_sin = np.corrcoef(d['zHD'].values, d['sin_th'].values)[0, 1]
print(f"corr(z, sinθ) = {rho_z_sin:.4f}  (다중공선성 확인 — |r|<0.3이면 문제 없음)")
...
t_r, p_r = stats.ttest_ind(r_ax, r_pp, equal_var=False)  # Welch t-test
...
for lo, hi in [(0.15,0.25),(0.25,0.35),(0.35,0.50)]:
    ...
    _, pb = stats.ttest_ind(ax_s, pp_s, equal_var=False)  # Welch t-test
...
print("="*60)
print("  (Greedy nearest-neighbor matching without replacement)")
print("  (Hungarian matching would give near-identical results)")
...
_, p_z_bal   = stats.ttest_ind(p_ax['zHD'], p_pp['zHD'], equal_var=False)  # Welch t-test
...
checks = [
    ...
    ('Analysis 1 N',         float(n),                  665.0,   1.0),
    ('Analysis 2 N pairs',   float(N),                  205.0,   1.0),
